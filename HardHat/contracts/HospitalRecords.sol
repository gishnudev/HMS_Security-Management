...
contract HospitalRecords {
    ...
    mapping(address => address) public assignedDoctor; // New: Track doctor assigned to patient
    ...
    event ReferralMade(address indexed patient, address indexed fromDoctor, address indexed toDoctor, uint32 recordId);
    event ReferralResponded(address indexed patient, uint indexed referralIndex, ReferralStatus status);
    event VitalsUpdated(uint32 indexed id, string vitals);
    event PrescriptionUpdated(uint32 indexed id, string prescription);
    ...
    function addPatientWithDoctor(address patient, string memory name, string memory id, address doctor) external only(Role.Receptionist) {
        require(roles[patient] == Role.None, "Already registered");
        require(roles[doctor] == Role.Doctor, "Invalid doctor");
        _addUser(patient, Role.Patient, Identity(name, id));
    ...
    function addRecordWithPreference(string memory ipfsHash, address preferredDoctor) external {
        if (roles[msg.sender] == Role.None) {
            _addUser(msg.sender, Role.Patient, Identity("AutoRegistered", "AUTO-ID"));
        }

        if (!isRegisteredPatient[msg.sender]) {
            isRegisteredPatient[msg.sender] = true;
        }

        address assigned = (preferredDoctor != address(0) && roles[preferredDoctor] == Role.Doctor)
            ? preferredDoctor
            : assignDoctorAutomatically();

        uint32 id = ++recordCount;
        records[id] = Record(id, ipfsHash, msg.sender, assigned, "", "", "", block.timestamp);
        patientRecords[msg.sender].push(id);
        emit RecordAdded(id, assigned, msg.sender);
    }
    ...
    function updateVitals(uint32 id, string memory vitals) external only(Role.Nurse) {
        records[id].vitals = vitals;
        emit VitalsUpdated(id, vitals);
    }

    function updatePrescription(uint32 id, string memory prescription, string memory notes) external only(Role.Doctor) {
        records[id].prescription = prescription;
        records[id].notes = notes;
        emit PrescriptionUpdated(id, prescription);
    }

    function viewRecord(uint32 id) external view returns (Record memory) {
        require(roles[msg.sender] != Role.None, "Unauthorized");
        return records[id];
    }
